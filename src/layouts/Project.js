    ...
    return (
        <>
            <section className="project-first-section">
                <div className="project-first-section__project__image" style={{
                    backgroundImage: `url(http://localhost:5000/${project.image})`
                }}/>
                <div className="project-first-section__project__info">
                    <h1 className="project-first-section__project__info__title">{project.title}</h1>
                    <p className="project-first-section__project__info__description">{project.description}</p>
                </div>
            </section>
            {/* code samples */}
            {project.codeSamples && project.codeSamples.length > 0 && 
            project.codeSamples.map((codeSample,index)=>(
                <CodeSamples key={index} codeSample={codeSample}/>
            ))
            }
            {/* carousel */}
            <Carousel/>
